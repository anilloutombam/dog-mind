...
export function UploadPanel(props: UploadPanelProps) {
  const { hasFile, fileName, previewUrl, status, error, result, onSelect, onAnalyze, onReset } = props;
  return (
    <div className="upload-panel">
      ...
      {status === "complete" && result ? <ResultCard analysis={result} previewUrl={previewUrl} onReset={onReset} />
        : <>
          ...
          {hasFile && status !== "analyzing" && <button className="analyze-button" onClick={onAnalyze}><WandSparkles size={18} /> Analyze my dog</button>}
        </>}
    </div>
  );
}
